...
export default function Services() {
  ...
  return (
    <section
      className="w-full px-4 py-10 sm:px-8 lg:px-20 bg-white dark:bg-gray-900 transition-colors duration-300"
      id="services"
    >
      {/* Section Heading */}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
        {services.map((service, index) => (
          <Link
            key={index}
            href={service.href}
            className="group bg-gray-100 dark:bg-gray-800 rounded-2xl p-6 hover:shadow-xl transition hover:bg-indigo-50 dark:hover:bg-indigo-700"
          >
            ...
            <p className="mt-4 text-indigo-600 dark:text-indigo-400 group-hover:underline text-sm">
              Learn more →
            </p>
          </Link>
        ))}
